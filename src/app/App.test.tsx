...
describe('App shell', () => {
  ...
  it('opens and closes update history while restoring trigger focus', async () => {
    ...
    expect(within(dialog).getAllByRole('listitem')[0]).toHaveTextContent('2026-08-31');
    expect(within(dialog).getAllByRole('listitem')[0]).toHaveTextContent('고를 말을 더 쉽게 찾고');
    expect(within(dialog).getAllByRole('listitem')[1]).toHaveTextContent('2026-08-29');
    expect(within(dialog).getAllByRole('listitem')[1]).toHaveTextContent('리디자인');
    expect(within(dialog).getAllByRole('listitem')[1]).toHaveTextContent('입구·활동 카드·기록 화면');
    expect(within(dialog).getAllByRole('listitem')[2]).toHaveTextContent('2026-08-28');
    expect(within(dialog).getAllByRole('listitem')[3]).toHaveTextContent('모바일 겹침을 없애고');
